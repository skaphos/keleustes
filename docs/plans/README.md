...
- 🟢 [2026-05-scale-benchmark-harness.md](./2026-05-scale-benchmark-harness.md) — **SKA-326.** Two complementary harnesses under `tools/benchmark/`: Go binary (CRD-aware workload generation + mock-agent publication + metrics scrape + report) plus K6 scripts (provider-HMAC webhook bursts + synthetic UI queries). Hybrid cluster fixture (kind for CI; real cluster for MVP exit). Pre-release-only CI cadence via `workflow_dispatch`. Mock agent for CI; real `internal/agent/` binaries for full benchmark. Absolute thresholds per MVP plus relative regression detection. Resolves runtime plan §13 Q13. Promotes to ADR 0012 (likely) when §14 open questions resolve and at least one MVP-exit gate has been run from a real release-manager workflow.
- 🟢 [2026-05-agent-transport-interface.md](./2026-05-agent-transport-interface.md) — **SKA-321.** Rich typed `internal/agent/transport.Transport` Go interface with NATS leaf as the only implementation through MVP 2; gRPC (SKA-378) and HTTP/2 long-poll sketched as alternates to prove interface generality. Outbound-only NKey+JWT auth where JWT carries `keleustes.deploymentTargets` allowlist. `Agent` CR spec'd. Deterministic 1:N agent:target claim via the same `controller-locks` NATS KV bucket from SKA-324 §6 / SKA-328 §5. NATS Object Store for large transient payloads. Pre-registered agents via `keleustesctl agent register`. Resolves runtime plan §13 Q8 + Q9. Promotes to ADR 0013 (likely) when §16 open questions resolve and SKA-363's MVP 2 implementation runs through the benchmark harness's real-agent profile.
