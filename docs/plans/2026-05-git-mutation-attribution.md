...
4. **First-time path:** no token found → Keleustes redirects Alice to GitHub's authorization URL for the Keleustes App (`https://github.com/login/oauth/authorize?client_id=…&redirect_uri=…&state=<promotion-name>`). Alice consents; GitHub redirects back to Keleustes with a code; Keleustes exchanges the code for an access token + refresh token; both are stored encrypted, scoped to Alice's actor subject.
